...
# =========================================================
# [신규 추가 - 범용(real-valued, 임의 파라미터) 버전]
# 위의 gaussian_log_likelihood 등은 (f_r,g,kappa) 3개 고정 + 복소수
# 데이터 전용입니다. 오늘 만든 TLS 2D 모델(5개 파라미터, 실수값 신호)
# 에는 안 맞아서, param_names를 명시적으로 받는 실수값 버전을
# 추가합니다. 기존 함수는 그대로 둬서 원래 avoided-crossing S21
# 예제에는 영향 없음.
# =========================================================

def _theta_to_kwargs_generic(theta, param_names):
    """param_names 목록을 그대로 받아 {이름: 값} 딕셔너리로 변환 -
    어떤 모델(파라미터 몇 개든)에도 재사용 가능."""
    return dict(zip(param_names, theta))


def gaussian_log_likelihood_2d_real(theta, model_func, model_kwargs, param_names,
                                       x_grid, y_grid, data_2d, sigma):
    """
    [신규] 2D 실수값 데이터용 가우시안 우도. TLS swap spectroscopy
    모델(tls_swap_spectroscopy_2d_model)처럼 복소수가 아니라 실수
    신호(dispamp)를 예측하는 모델에 맞춤.
    """
    model = model_func(x_grid, y_grid, **model_kwargs, **_theta_to_kwargs_generic(theta, param_names))
    residual = data_2d - model
    chi2 = np.sum((residual / sigma) ** 2)
    return -0.5 * chi2


def robust_log_likelihood_2d_real(theta, model_func, model_kwargs, param_names,
                                     x_grid, y_grid, data_2d, sigma, nu=4.0):
    """[신규] 2D 실수값 데이터용 robust(Student-t) 우도."""
    model = model_func(x_grid, y_grid, **model_kwargs, **_theta_to_kwargs_generic(theta, param_names))
    residual = data_2d - model
    t_term = np.log1p((residual / sigma) ** 2 / nu)
    return -0.5 * (nu + 1) * np.sum(t_term)


def make_log_probability_2d_real(model_func, model_kwargs, param_names, log_prior_func,
                                    likelihood_type='gaussian', **likelihood_kwargs):
    """
    [신규] 2D 실수값 모델 전용 log_probability 팩토리. 반환 함수
    시그니처는 log_probability(theta, x_grid, y_grid, data_2d, sigma) -
    emcee가 있는 환경(Colab)에서 mcmc_pipeline에 그대로 넘겨 진짜
    MCMC로 바로 업그레이드 가능.
    """
    if likelihood_type == 'gaussian':
        likelihood_func = gaussian_log_likelihood_2d_real
    elif likelihood_type == 'robust':
        likelihood_func = robust_log_likelihood_2d_real
    else:
        raise ValueError(f"알 수 없는 likelihood_type: {likelihood_type}")

    def log_probability(theta, x_grid, y_grid, data_2d, sigma):
        lp = log_prior_func(theta)
        if not np.isfinite(lp):
            return -np.inf
        ll = likelihood_func(theta, model_func, model_kwargs, param_names,
                               x_grid, y_grid, data_2d, sigma, **likelihood_kwargs)
        return lp + ll
