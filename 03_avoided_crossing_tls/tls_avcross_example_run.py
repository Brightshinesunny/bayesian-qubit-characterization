...
import tls_avcross_fisher_matrix as fm
...
# =========================================================
np.savez(
    os.path.join(aa_output_dir, f'results_{aa_likelihood_type}_outlier{aa_outlier_probability}.npz'),
        # [수정] 파일명에 likelihood_type과 outlier_probability를 포함시켜서,
        # gaussian으로 한 번, robust로 한 번 실행해도 서로 다른 파일로 저장되게 함
        # (이전에는 항상 같은 이름이라 두 번째 실행이 첫 번째 결과를 덮어썼음).
    true_fr=aa_f_r0, true_g=aa_g, true_kappa=aa_kappa,
        # 비교 스크립트(compare_gaussian_vs_robust.py)가 참값을 다시 몰라도
        # 되도록, 참값 자체도 결과 파일 안에 같이 저장해둠.
    **results
)
