"""
Fano / TLS / baseline-tilt 주입 스트레스 테스트 (수정본)

원본 대비 주요 변경점
  [FIX-1] prior 상한 확대 — g, kappa 가 0.1 벽에 붙어 있었음
  [FIX-2] walker 초기 산포를 파라미터별 상대값으로 — 절반이 prior 밖에서 시작했음
  [FIX-3] 딥 탐색을 detrend + prominence + distance 기준으로 교체
  [FIX-4] MCMC 시드 고정 (재현성)
  [FIX-5] median 대신 MAP 으로 최적 곡선 표시 (median 도 병기)
  [FIX-6] prior 경계 접촉 자동 검사
  [FIX-7] tau vs N/50 수렴 기준 명시적 검사
  [FIX-8] moving average 길이를 입력과 동일하게 고정
  [FIX-9] 반교차 flux 지점 자동 탐색 (Φ=0 이 dispersive 영역일 가능성)
  [FIX-10] 시나리오 개별 스윕 모드 — 각 효과의 기여를 분리
"""
...
aa_tls_linewidth = 0.006   #       현실적 강도를 보려면 0.05~0.15 도 함께 시험할 것.
...
aa_nwalkers = 32
aa_nsteps = 8000           # [FIX-7] 800 -> 8000. tau~90 이면 50*tau=4500 필요
aa_burn_in_discard = 2000
aa_thin_by = 10

# [FIX-2] 파라미터별 상대 산포. g_guess 가 1e-3 수준이어도 prior 밖으로 안 나감
...
def moving_average_same(x, window):
    ...
    원본은 diagnostics.moving_average 결과를 f_grid 로 인덱싱했는데,
    그 함수가 mode='valid' 로 길이를 줄여 반환하면 window//2 만큼
    주파수가 어긋난다. 여기서는 edge padding 으로 길이를 맞춘다.
    """
    if window < 2:
...
def detrended_magnitude(f_grid, s21_1d, window):
    ...
    tilt 가 켜져 있으면 '절대적으로 가장 낮은 점' 은 공진이 아니라
...
def find_dips(f_grid, s21_1d, window,
    ...
    if verbose:
        ...
        for i, p in list(zip(idx_sorted, prom_sorted))[:5]:
            print(f"    f={f_grid[i]:.4f} GHz  |S21|={mag_s[i]:.4f}  prominence={p:.4f}")
...
def initial_guess_from_dips(f_grid, s21_1d, window, bounds, verbose=True):
    ...
    if len(idx) >= 2:
        ...
        # 딥이 하나면 반교차가 분해되지 않은 것. 폭에서 g 를 짐작할 수 없으므로
        # prior 중앙값을 쓰고, 그 사실을 명시한다.
        fr_g = float(f_grid[idx[0]])
        g_g = float(np.sqrt(bounds['g'][0] * bounds['g'][1]))  # 로그 중앙
...
def make_init_scatter(guess):
    ...
    return np.maximum(np.abs(guess) * aa_init_scatter_rel, aa_init_scatter_floor)
...
def get_map_theta(result, fallback):
    ...
    median 은 성분별 중앙값이라, posterior 가 비대칭이면
    실제로는 어느 표본과도 일치하지 않는 점이 될 수 있다.
    """
...
def build_signal(flux_val, use_fano, use_tls, use_tilt, seed=aa_seed):
    ...
    return f_grid, s21, s21_noisy, effects
...
def find_anticrossing_flux(n_scan=161, flux_range=(-0.5, 0.5)):
    """왜곡 없는 clean 신호로 flux 를 훑어, 딥이 2개로 갈라지고
    그 간격이 최소가 되는 지점을 찾는다. 그곳이 반교차이며 간격 ~ 2g.

    원본의 Φ=0 은 표준 transmon 이라면 fq(0)=fq_max=5.150 이고
    f_r0=5.000 이므로 detuning=0.150=3.75g 인 dispersive 영역이다.
    그 경우 딥은 하나만 나오고 '딥 2개 찾기' 는 성립하지 않는다.
    """
    print("\n[반교차 flux 탐색] clean 신호로 스캔합니다...")
    f_grid = np.linspace(*aa_freq_range, aa_n_freq)
    best = None
    two_dip_fluxes = []
    ...
    print(f"  최소 간격 지점: flux={flux_best:+.4f}, 간격={sep_best:.4f} GHz "
          f"(2g={2*aa_g:.4f} 와 비교)")
...
def run_case(flux_val, use_fano, use_tls, use_tilt, tag, make_plot=True):
    ...
    f_grid, s21_clean, s21_noisy, effects = build_signal(
    ...
    print(f"\n  noise_sigma 추정={sigma_est:.5f}  참값={aa_white_noise_level}  "
          f"비율={ratio:.2f}x")
    if ratio > 1.5:
        print("  [주의] 잡음을 과대평가하면 尤度가 느슨해져, 잔차가 커도")
    ...
    # ---- 결과 ----
    print("\n  " + "-" * 58)
    print(f"  {'param':8s} {'median':>10s} {'MAP':>10s} {'true':>10s} {'err(med)':>10s}")
    print("  " + "-" * 58)
    errs = {}
    for i, name in enumerate(PARAM_NAMES):
    ...
    if hits:
        ...
        print("    -> 이 추정값은 '데이터가 말한 값' 이 아니라 '탐색 범위의 끝' 입니다.")
    ...
    if make_plot:
        ...
        axes[0].plot(f_grid, np.abs(s21_noisy), '.', ms=3, alpha=0.5,
                     label='Measured (distorted)')
        axes[0].plot(f_grid, np.abs(s21_clean), '-', lw=1, alpha=0.6,
                     color='green', label='Clean (no distortion)')
