...
8/21〜8/29にかけて行った全作業（130以上のファイル）をプロジェクト
ごとに整理したものです。**どのファイルから見ればよいか分からない
場合は、まずこの文書をお読みください。**
...
| `09_summaries/SUMMARY_robots_performance_and_full_journey.py` | 検証ロボット8種の実戦性能評価＋全体行程の統合版 |

---

## 01_zenodo_baseline — 基礎パイプライン（8/21〜8/23）
...
## 02_fano_resonator — Fano共振器、circle fit vs ベイズ/フィッシャー（8/23〜8/27）
...
## 03_avoided_crossing_tls — avoided-crossing 復習＋TLS拡張（8/28）
...
## 04_tls_data_structure — TLSデータ構造の解読（8/27）
...
## 05_tls_noise_diagnosis_dip_tracking — 系統誤差診断＋ディップ追跡（8/27）
...
## 06_tls_multi_file_integration — TLS3ファイルの統合（8/29）
...
## 07_catalog_to_raw_mapping — カタログ↔raw 逆マッピング（8/29）
...
## 08_validation_robots — 検証ロボット（8/28〜8/29）

8種のロボット（prior妥当性チェック／パラメータ復元／multi-start
...
2. **再利用できるコード**：`08_validation_robots/tls_avcross_pipeline.py`（avoided-crossing）、
   `08_validation_robots/validation_robots.py`（検証）、
