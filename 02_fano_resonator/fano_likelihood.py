"""
fano_likelihood.py  (likelihood.py를 Fano 데이터 워크플로우 전용으로 복제한 버전 - 이름 충돌/캐시 문제 방지용)
=================================================================
...
# =========================================================
# [신규 추가] 범용(param_names 기반) 버전
# =========================================================
# 위의 gaussian_log_likelihood / robust_log_likelihood / make_log_probability는
# _theta_to_kwargs가 (f_r, g, kappa) 3개로 하드코딩되어 있어서, Sij처럼
# 파라미터 이름/개수가 다른 모델(fr, Ql, Qc, phi, a, alpha 6개)에는 그대로
# 못 씁니다. 어제 Zenodo 데이터 작업에서 zenodo_fisher_matrix.py를
# "파라미터 이름 목록을 인자로 받는" 범용 설계로 다시 만들었던 것과
# 똑같은 이유로, 여기서도 param_names를 명시적으로 받는 범용 버전을
# 추가합니다. 기존 함수들은 그대로 남겨둬서(하위 호환), avoided-crossing
# 템플릿을 쓰는 다른 스크립트에는 영향이 없습니다.

def _theta_to_kwargs_generic(theta, param_names):
    """
    _theta_to_kwargs의 범용 버전. 파라미터 이름 목록을 직접 받아서
    {이름: 값} 딕셔너리를 만듦 - 어떤 물리 모델(파라미터 몇 개든)에도
    그대로 재사용 가능.
    """
    return dict(zip(param_names, theta))


def gaussian_log_likelihood_generic(theta, model_func, model_kwargs, param_names,
                                       f_grid, data_1d, sigma):
    """gaussian_log_likelihood의 범용 버전 (param_names를 명시적으로 받음)."""
    model = model_func(f_grid, **model_kwargs, **_theta_to_kwargs_generic(theta, param_names))
    real_res = np.real(data_1d - model)
    imag_res = np.imag(data_1d - model)
    chi2 = np.sum((real_res / sigma) ** 2 + (imag_res / sigma) ** 2)
    return -0.5 * chi2
...
def robust_log_likelihood_generic(theta, model_func, model_kwargs, param_names,
                                     f_grid, data_1d, sigma, nu=4.0):
    """robust_log_likelihood의 범용 버전."""
    model = model_func(f_grid, **model_kwargs, **_theta_to_kwargs_generic(theta, param_names))
    real_res = np.real(data_1d - model)
    imag_res = np.imag(data_1d - model)
    ...
    return -0.5 * (nu + 1) * np.sum(t_term(real_res) + t_term(imag_res))


def make_log_probability_generic(model_func, model_kwargs, param_names, log_prior_func,
                                    likelihood_type='gaussian', **likelihood_kwargs):
    """
    make_log_probability의 범용 버전. 반환되는 함수의 시그니처는 원본과
    똑같이 log_probability(theta, f_grid, data_1d, sigma)이므로,
    fano_mcmc_pipeline.run_single_mcmc에 그대로 꽂아 쓸 수 있습니다
    (emcee가 설치된 환경, 예: Colab에서 실제 MCMC를 돌릴 때).

    사용 예 (Sij 6개 파라미터 모델):
        log_prior = make_uniform_log_prior({
            'fr': (9.4689e9, 9.4729e9), 'Ql': (100,100000), 'Qc': (100,100000),
            'phi': (-np.pi,np.pi), 'a': (1e-4,0.2), 'alpha': (-np.pi,np.pi)
        })
        log_prob = make_log_probability_generic(
            fano_models.Sij, {'delay': cable_delay, 'n_ports': 1.0},
            ['fr','Ql','Qc','phi','a','alpha'], log_prior, likelihood_type='gaussian'
        )
    """
    if likelihood_type == 'gaussian':
        likelihood_func = gaussian_log_likelihood_generic
    elif likelihood_type == 'robust':
        likelihood_func = robust_log_likelihood_generic
    else:
        raise ValueError(f"알 수 없는 likelihood_type: {likelihood_type}")
    ...
    def log_probability(theta, f_grid, data_1d, sigma):
        ...
        if not np.isfinite(lp):
            return -np.inf
        ll = likelihood_func(theta, model_func, model_kwargs, param_names, f_grid, data_1d, sigma,
                               **likelihood_kwargs)
        return lp + ll
